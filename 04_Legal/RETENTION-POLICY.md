...
| Check-in token | `registrations` → `check_in_token` | **Delete the day after the event** | It is a credential. Holding a live credential after it can be used is pure risk. |
| **Attendance record — when someone arrived and which volunteer checked them in** | `registrations` → `checked_in_at`, `checked_in_by` | **With the registration: 12 months after the event** | Added 2026-09-03 with the QR arrival desk. Deliberately given no clock of its own: it is the record of who was in the building, so it must outlive the day and must not outlive the record of who was expected. `checked_in_by` is a moderator's player id, not a name — the minimum that answers "who was on the desk?" if the register turns out to be wrong. Note it survives the token above, which is correct: the credential expires, the fact of attendance does not. |
| **Proof of date of birth — that it was seen, when, and by which moderator** | `registrations` → `dob_verified_at`, `dob_verified_by` | **With the registration: 12 months after the event** | Added 2026-09-03 when the team made proof of date of birth a condition of checking in. **The document itself is never recorded in any form** — not its type, not its number, not an image, and not the date read off it. It is looked at and handed straight back; these two columns hold only that the check happened. There is deliberately no column for anything else and a test asserts none exists. |
| **Printed check-in slips** | Paper, in the venue | **Destroyed at the end of the event** | The only physical personal data the project creates: one card per player carrying their public name, their reference and a live check-in code. Not in any system, so no code can enforce this — it is on the `/admin` print sheet as an instruction and on the on-the-day checklist. Leftovers go in a bag and are binned, not filed. |
...
| Check-in tokens cleared the day after the event | `clearCheckInTokens()`. Clears the credential and leaves `checked_in_at` alone, on purpose: the pass stops working, the fact that somebody attended stays until the registration goes |
