...
limited public subset (first name, avatar, region, age band), email notification to the
...
| 13 | **An account is held for a child who never attended an event** | High | Low–Medium | Introduced deliberately in round 42 (see section 2). A profile holds first name, email, age band, date of birth, region and avatar — no health data, which lives on the registration and is purged separately. The person asked for the profile, it does something for them, and it is deletable on request through `/support`. **Honestly stated: this is more children's data held for longer than before the change**, and it is proportionate only while retention is enforced. `[NEEDS: a retention duration for a profile with no attended event — proposed: delete after 24 months of no activity.]` | `[Medium until the duration is set and enforced]` |
...
4. **#13 — set a retention duration for a profile that never attended an event.** Proposed:
   delete after 24 months of no activity. This is the one genuinely new question created by
   the round 42 change, and it needs an answer, not a mitigation.
...
**This document has never been signed.** Items 1–4 above should be settled and written in
